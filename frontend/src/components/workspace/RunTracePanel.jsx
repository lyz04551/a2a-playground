...
export default function RunTracePanel({ run = {}, stage, agents = [], language = 'en-US', loading = false, error, canCancel = false, cancelling = false, onCancel, onApproval, onArtifactOpen, onDebug }) {
  ...
  return (
    <aside className="workspace-trace" aria-label="Run trace">
      ...
      <RunTimeline run={run} tasks={tasks} rounds={run.rounds || []} selectedTaskId={selectedTaskId} onTaskSelect={task => setSelectedTaskId(task.id)} language={language} />
      {approvals.map((approval, index) => <ApprovalCard key={approval.id} approval={approval} previousApproval={previousMatchingApproval(approval, index)} language={language} onDecide={onApproval} />)}
