...
提出加固配置时必须保持运行时语义完整，适用于所有 Kubernetes workload：
- `containerPort` 只是元数据，不会改变进程实际监听端口；端口调整必须有镜像能力、启动参数或配置文件证据。
- `runAsNonRoot`、只读根文件系统、volumeMount、command/args 等建议必须与目标镜像的 UID、文件权限和启动行为兼容。
- 不得用空卷覆盖镜像必需的默认配置目录，除非同时提供可工作的配置来源。
- 缺少上述兼容性证据时，将建议标记为“需验证”，不要声称配置可正常运行；优先推荐明确支持目标安全模型的镜像或最小必要配置。
